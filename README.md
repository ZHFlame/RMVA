...
- **算法**：使用 **`cv::solvePnP`**（Method：**`SOLVEPNP_IPPE`**），对平面目标（装甲板）具有更好的稳定性。  
- **目标模型**：基于比赛规则的大装甲板尺寸，长 **0.705 m**，宽（高）**0.230 m**。  
- **点序定义**：严格对应检测节点输出顺序：**左下 → 右下 → 右上 → 左上**（逆时针）。  
- **输出**：装甲板中心相对于相机光心的三维坐标 **$(x, y, z)$**。

#### 3. EKF 运动预测 (`CorrectCircularMotionEKF`)

针对赛场中装甲板的旋转运动，引入扩展卡尔曼滤波（EKF）进行轨迹预测。
...
- **预测流程**：  
  ...
  2. **时间估计**：计算子弹飞行时间 **$t_{fly} = \dfrac{\sqrt{x^2 + y^2 + z^2}}{v}$**；  
...
- **Yaw（偏航角）**：
  $$
  \psi = \operatorname{atan2}(x_{pred}, z_{pred})
  $$

- **Pitch（俯仰角）**：建立抛物线模型，解算一元二次方程：
  $$
  ...
  - **$v$**：当前弹速（默认为 **15 m/s**，需按实际调整）；  
  - **$g$**：重力加速度。

  **求解策略**：  
  - 若方程有解（判别式 **$\Delta \ge 0$**）：取较小的 **$\theta$**（直射弹道，飞行时间短）；  
  - 若方程无解（超出射程）：退化为直接指向目标：
    $$
    \theta = \operatorname{atan2}(x_{pred}, z_{pred})
    $$
